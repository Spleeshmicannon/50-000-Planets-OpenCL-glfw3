...
__kernel void planetCalc(__global float planet[MATRIX_HEIGHT][MATRIX_WIDTH], __global float outPlanet[MATRIX_HEIGHT][2], int num_threads, int itemsPerThread)
{
	// Get the index of the current element to be processed
	__private float fx = 0, fy = 0;

	__private const int startIndex = itemsPerThread * get_global_id(0);
	__private const int endIndex = min(startIndex + itemsPerThread, MATRIX_HEIGHT);
